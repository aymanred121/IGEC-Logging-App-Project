    ...
    private void initialize(View view) {
        ...
        if (isProject) {
            ...
            db.collection("EmployeesGrossSalary").document(employee.getId()).collection(year).document(month).addSnapshotListener((value, error) -> {
                ...
                // get (penalty || gift )
                employeesGrossSalary.getAllTypes().stream().filter(allowance -> allowance.getType() == allowancesEnum.RETENTION.ordinal()).forEach(allowance -> allowances.add(allowance));
                // get (bonus)
